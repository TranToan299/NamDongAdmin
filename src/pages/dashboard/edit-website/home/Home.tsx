...
export default function Home() {
  ...
  // store
  const { listProjectImage, listHomePageImage } = useSelector((state) => state.projectImage);
  //
  ...
  const defaultValues: { projectImage: IProjectImage[] | IRenderProjectImage[] | [] } = {
    // projectImage: [],
    projectImage: listHomePageImage,
  };
  ...
  useEffect(() => {
    if (listHomePageImage && listHomePageImage.length) {
      // handle change images from string to array
      const listHomePageImageTemp = listHomePageImage.map((item: IRenderProjectImage) => {
  ...
  // useEffect(() => {
  //   console.log("listpage: ",listHomePageImage);

  // }, []);

  useEffect(() => {
    if (!listHomePageImage.length) {

...
export default function Store() {
  ...
  // store
  const { listProjectImage, listStorePageImage } = useSelector((state) => state.projectImage);
  //
  ...
  const defaultValues: { projectImage: IProjectImage[] | IRenderProjectImage[] | [] } = {
    // projectImage: [],
    projectImage: listStorePageImage,
  };
  ...
  useEffect(() => {
    if (listStorePageImage && listStorePageImage.length) {
      // handle change images from string to array
      const listHomePageImageTemp = listStorePageImage.map((item: IRenderProjectImage) => {
  ...
  // useEffect(() => {
  //   console.log("listpage: ",listStorePageImage);

  // }, []);

  useEffect(() => {
    if (!listStorePageImage.length) {

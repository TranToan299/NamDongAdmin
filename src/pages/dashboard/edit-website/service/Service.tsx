...
export default function Service() {
  ...
  // store
  const { listProjectImage, listServicePageImage, editorWebsiteAndSystem } = useSelector((state) => state.projectImage);
  //

  // State
  const [editValue, setEditValue] = useState('');
  console.log("🚀 ~ file: Service.tsx:50 ~ Service ~ editValue:", editValue)
  ...
  const defaultValues: { projectImage: IProjectImage[] | IRenderProjectImage[] | [] } = {
    // projectImage: [],
    projectImage: listServicePageImage,
  };
  ...
  const onSubmit = async (data: { projectImage: IProjectImage[] | IRenderProjectImage[] }) => {
    ...
    if(editorWebsiteAndSystem && editValue !== editorWebsiteAndSystem.description){
      ProjectImageApi.post([{
  ...
  const getEditoWebsiteAndSystemHandle = async() => {
    const res = await dispatch(getEditorWebsiteAndSystem());
    if(res){
      setEditValue(res.payload[0].description)
    }
  };

  useEffect(()=>{
    // if(!editorWebsiteAndSystem){
      getEditoWebsiteAndSystemHandle()
    // }
  },[])

  useEffect(() => {
    if (listServicePageImage && listServicePageImage.length) {
      // handle change images from string to array
      const listHomePageImageTemp = listServicePageImage.map((item: IRenderProjectImage) => {
  ...
  // useEffect(() => {
  //   console.log("listpage: ",listServicePageImage);

  // }, []);

  useEffect(() => {
    if (!listServicePageImage.length) {
    ...
    <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
      <PageWrapper title={t('pageImage')}>
        <Container maxWidth={themeStretch ? false : 'lg'}>
          ...
          <Card sx={{ mt: 3, p: 3 }}>
          <Typography variant="h6" sx={{marginBlock: 3}}>{t('websiteAndSystem')}</Typography>

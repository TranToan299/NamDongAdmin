...
export default function Contact() {
  ...
  // store
  const { listProjectImage, listContactPageImage } = useSelector((state) => state.projectImage);
  //
  ...
  const defaultValues: { projectImage: IProjectImage[] | IRenderProjectImage[] | [] } = {
    // projectImage: [],
    projectImage: listContactPageImage,
  };
  ...
  useEffect(() => {
    if (listContactPageImage && listContactPageImage.length) {
      // handle change images from string to array
      const listHomePageImageTemp = listContactPageImage.map((item: IRenderProjectImage) => {
  ...
  // useEffect(() => {
  //   console.log("listpage: ",listContactPageImage);

  // }, []);

  useEffect(() => {
    if (!listContactPageImage.length) {

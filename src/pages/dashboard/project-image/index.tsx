...
export default function ProjectImagePage() {
  ...
  // store
  const { listProjectImage } = useSelector((state) => state.projectImage);
  //
  ...
  useEffect(() => {
    if (listProjectImage) {
      ...
      const listProjectImageTemp = listProjectImage.map((item: IRenderProjectImage) => {
    ...
    <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
      <PageWrapper title={t('projectImage')}>
        <Container maxWidth={themeStretch ? false : 'lg'}>
          <CustomBreadcrumbs
            heading={t('settingProjectImage')}
            links={[
              { name: t('dashboard'), href: PATH_DASHBOARD.root },
              { name: t('projectImage'), href: PATH_DASHBOARD.app.projectImage.projectImage },
              { name: t('list') },
            ]}
            action={
              <Button
